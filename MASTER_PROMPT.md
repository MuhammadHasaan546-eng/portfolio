...
- **Resume:** `/resume.pdf` (public asset, opens in a new tab / downloads)
- **Profile photo:** `/profile.jpg` (public asset, rendered via `next/image`)
...
5. **Animated entrance splash** — full-screen `bg-[#111111]` overlay at `z-[150]` that
...
1. **Header (floating, `z-50`)** — pill nav with backdrop blur; logo “HASAAN”;
   center links: Work / About / Services / Skills / Experience / Contact; a live
...
2. **Hero** — oversized “DEVELOPER” watermark behind content (`z-0`), left column
   (intro badge, headline, bio, CTAs **including a “Resume” download link**), center
   portrait card showing `profile.photoUrl`, right social rail. Uses
   `profile.totalExperienceYears` (“2+”).
3. **About** — the developer behind the work. Large portrait (`profile.photoUrl`,
   `next/image` with `fill` + `sizes`, initials fallback behind it), a display-font bio,
   a facts grid (location / experience / focus / availability) and a prominent
   **“Download Resume”** button linking to `profile.resumeUrl` (`target="_blank"`,
   `download`). Index `01`.
4. **Selected Work** — filter tabs (All / AI Apps / E-Commerce / Web Platforms) with a
...
5. **Services** — hover-driven accordion (3 services) that inverts to dark on open,
   with a floating preview image and tech tags.
6. **Skills** — scrolling marquee of the full tech list + grouped skill cards.
7. **Experience** — dark (`#111111`) timeline with `kind` badges
   (Work / Workshop / Open Source) and a cursor-following preview tooltip.
8. **CTA / Footer** — “HAVE A PROJECT IN MIND?”, copy-to-clipboard email, “Contact Me”
...
- **`HoverPreviewText.jsx`** — reusable, prop-driven component that wraps inline text
  (`children`) and reveals a cursor-following preview card on hover (`pointer-events-none
  fixed z-50 rounded-2xl overflow-hidden shadow-2xl border border-white/20`). Props:
  `children`, `images` (array of `{ src, label }`), `className`, `offset` (`{x,y}`),
  `as` (wrapper tag). Uses `useMotionValue` + `useSpring` for fluid delayed tracking and
  cycles through multiple images as the pointer travels. Wired into the Experience header
  (“crafting the web”), the Services header (“for your product”) and each service title
  (Full-Stack Web Dev / AI Integration / REST APIs).
- **`Splash.jsx`** — animated entrance reveal (see Global Behaviour #5).
- **`CustomCursor.jsx`**, **`SmoothScroll.jsx`**, **`GridLines.jsx`** — global behaviour.

## DATA ARCHITECTURE

All content lives in **`src/data/portfolio.js`** and is imported by components:
`profile` (incl. `resumeUrl: "/resume.pdf"` and `photoUrl: "/profile.jpg"`), `navLinks`,
`projects`, `projectCategories`, `skillGroups`, `services`, `experience`, `siteUrl`,
`getProject(slug)`, and `previews` (hover-preview artwork keyed by section / service id /
company). Placeholder artwork is generated by a deterministic `makeArt(colors, seed)` SVG
data-URI helper — **all `#` must be encoded to `%23` and spaces to `%20`** or the image
truncates. Hover-preview images live in **`/public/previews/`** (`web-development.jpeg`,
`rest-api.jpg`, `cloud-security.jpeg`, `ai-integration.jpeg`, `creator-studio.jpeg`,
`north-studio.png`, `freelance.png`).
...
7. Resume and profile photo are first-class assets in `/public` (`/resume.pdf`,
   `/profile.jpg`); the resume link opens in a new tab with `download`.
8. Build and lint must pass clean (`npm run build && npm run lint`) before finishing.
